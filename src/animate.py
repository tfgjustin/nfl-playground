...
def main(argv):
    ...
    print('Loaded data for %d games' % len(games_df))
    plays_df = load_plays_data(games_df, argv[4])
    print('Loaded data for %d plays' % len(plays_df))
    players_df = normalize_column_formatting(pd.read_csv(argv[5]))
    print('Loaded data for %d players' % len(players_df))
    tracking_df = load_all_tracking_data([argv[6]])
    print('Loaded data for %d tracking points' % len(tracking_df))
    pff_df = try_read_pff(argv[7])
    if pff_df is not None:
        print('Loaded data for %d PFF players' % len(pff_df))
    else:
        print('Loaded data for 0 PFF players')
    tracking_df = standardize_tracking_dataframes(games_df, plays_df, tracking_df, pff_df=pff_df, players_df=players_df)
    print('Standardized %d frames' % len(tracking_df))
